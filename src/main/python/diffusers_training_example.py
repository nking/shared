...
NOTE: after training the model, the code gets memory intensive and so this should
be run on a platform with alot of memory (RAM and disk space are used for 
its memory).
...
if run_small_inspect:
    ...
    # save last 2 images for a test dataset used to see de-noise of real images
    ind = np.array([i for i in range(0, len(dataset)-2)], dtype=np.int32)
    # include the last training image in test too to look at effect on known image
    ind2 = np.array([i for i in range(len(dataset) - 3, len(dataset))], dtype=np.int32)
    indices = torch.from_numpy(ind)
...
if True:
    ...
    # if running out of memory (RAM + disk) in filter_noise, so set this lower:
...
def filter_noise(add_noise=False):
    ...
    print(f'apply de-noise filtering to test images (1 is from train, 2 are not)')

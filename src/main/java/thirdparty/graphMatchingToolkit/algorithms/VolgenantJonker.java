...
public class VolgenantJonker {
     ...
     * @param rowsol output column assigned to row in solution
       e.g. job i is assigned to worker rowsol[i], where numeration is from 0 on
     ...
     * @return 
     */
    protected double lap(int dim, double[][] assigncost, int[] rowsol, int[] colsol,
        double[] u, double[] v) {
    ...
    /**
     * get the row assigned to the column of original matrix.
     * @return 
     */
    ...
    public int computeAssignment(int[][] matrix) {
        ...
        double cost = lap(dim, costMatrix, rowsol, colsol, u, v);
